 ...
 * 字符串流
 *
 * <p>字符串流是为了更好地处理字符串，它内部存储了一个原始{@link String 字符串}的对象，
 * 以及这个字符串的起始下标（beginIndex）、终止下标的后一位（endIndex）、一个指向当前字符元素的游标（cursor），另外，它内部还存储
 * 了该字符串中出现的成对的引号的范围、括号的范围。
 */
