...
public abstract class AbstractIndices<E>
    ...
    public void update(E before, E after) {
        ...
        for (Map.Entry<String, Index<E>> entry : indices.entrySet()) {
            entry.getValue().update(before, after);
        }
    }
    ...
    @Override
    public QueryResult<E> query(LogicalExpression expression) {
        ...
        if (expression.isAlwaysTrue() || expression.isAlwaysFalse()) {
            ...
            if (singles == null || singles.isEmpty()) {
                ...
                Index<E> index = singles.get(keyValue.getLeftValue().asKey());
                if (index == null) {
            ...
            LogicalOperator link = multi.getLogicalOperator();
            ...
            LogicalExpression left = multi.getLeftExpression();
            LogicalExpression right = multi.getRightExpression();
            ...
            if (left.isSimple() && right.isSimple() && link.isAnd()) {
                ...
                if (multis == null || multis.isEmpty()) {
                    ...
                    Collection<MultiIndex<E>> _indices = multis.get(_left.getLeftValue().asKey());
                    if (_indices == null) {
                        ...
                        for (MultiIndex<E> _index : _indices) {
                            if (_index.indexOf(_right.getLeftValue().asKey()) != -1
