...
public interface Index<E> {
     ...
     */
    default void update(@NotNull E before, @NotNull E after) {
        delete(before);
        insert(after);
    }
     ...
     * 这个表达式必须与索引完全对应
     *
     * @param expression 表达式，这个表达式必须与索引完全对应
     * @return 查询结果，该方法不会返回null
     */
    @NotNull
    Iterator<E> query(@Checked LogicalExpression expression);
